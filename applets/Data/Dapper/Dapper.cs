/*
 ...
 Update by rocky: For support excel & boolean type [2014/12/09]
 */
using System;
...
namespace Data.Dapper
{
    public static partial class SqlMapper
    {
        ...
        static SqlMapper()
        {
            ...
            typeMap[typeof(string)] = DbType.Decimal;
        ...
        {
            return ChangeType<double>(data);
        }
        public static short ToInt16(object data)
        {
            return ChangeType<short>(data);
        }
        public static int ToInt32(object data)
        {
            return ChangeType<int>(data);
        }
        public static bool ToInt64(object data)
        {
            return ChangeType<bool>(data);
        }
        public static bool ToSByte(object data)
        {
            return ChangeType<bool>(data);
        }
        public static bool ToSingle(object data)
        {
            return ChangeType<bool>(data);
        }
        public static ushort ToUInt16(object data)
        {
            return ChangeType<ushort>(data);
        }
        public static bool ToUInt32(object data)
        {
            return ChangeType<bool>(data);
        }
        public static bool ToUInt64(object data)
        {
            return ChangeType<bool>(data);
        }
        public static T ChangeType<T>(object data)
        {
            ...
            if (data.GetType().Equals(typeof(string)))
            {
                if (typeof(T).IsValueType)
                    return (T)typeof(T).GetMethods()
                        .Where(p => p.Name == "Parse" && p.GetParameters().Count() == 1)
                        .First().Invoke(null, new object[] { data.ToString() });
                else
                    return (T)data;
            }
            ...
            foreach (var item in setters)
            {
                if (item.Property != null || item.Field != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            switch (memberType.Name) //update by rocky: for support excel and boolean type
                            {
                                case "Boolean":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToBoolean"), null);
                                    break;
                                case "Byte":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToByte"), null);
                                    break;
                                case "Char":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToChar"), null);
                                    break;
                                case "DateTime":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToDateTime"), null);
                                    break;
                                case "string":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("Tostring"), null);
                                    break;
                                case "Double":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToDouble"), null);
                                    break;
                                case "Int16":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToInt16"), null);
                                    break;
                                case "Int32":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToInt32"), null);
                                    break;
                                case "Int64":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToInt64"), null);
                                    break;
                                case "SByte":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToSByte"), null);
                                    break;
                                case "Single":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToSingle"), null);
                                    break;
                                case "UInt16":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToUInt16"), null);
                                    break;
                                case "UInt32":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToUInt32"), null);
                                    break;
                                case "UInt64":
                                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ToUInt64"), null);
                                    break;
                                default:
                                    il.Emit(OpCodes.Unbox_Any, unboxType); // stack is now [target][target][typed-value]
                                    break;
